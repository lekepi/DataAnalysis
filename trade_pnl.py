...
def get_full_pnl(start_date, end_date):
    ...
    my_sql = f"""SELECT T1.id,trade_date,T2.ticker,T1.product_id,T1.notional_usd,pnl_close FROM trade T1
                JOIN product T2 on T1.product_id=T2.id WHERE trade_date>='{start_date}' and trade_date<'{end_date}'
                 and prod_type in ('Cash', ' Future') and parent_fund_id=1 order by trade_date;"""
