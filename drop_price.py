...
from utils import find_past_date, find_future_date, clean_df_value
...
def get_drop():
    ...
    drop_list = session.query(ProductBeta).filter(ProductBeta.alpha <= -0.05).\
        order_by(ProductBeta.entry_date.asc()).all()
    total = len(drop_list)
    count = 0
    for drop in drop_list:
        count += 1
        entry_date = drop.entry_date
        ...
        continent = drop.product.exchange.country.continent
        alpha = drop.alpha
        ticker = drop.product.ticker
        if drop.product.exchange.country.continent == 'AMER':
            df_index = df_spx
        else:
            df_index = df_sxxp

        position = session.query(Position).filter(Position.entry_date == entry_date).\
            filter(Position.product_id == drop.product.id).filter(Position.parent_fund_id == 1).first()
        ...
        # MA 50D
        my_sql = f"""SELECT entry_date,adj_price as price FROM anandaprod.product_market_data WHERE product_id={drop.product.id}
        ...
        price_1d = clean_df_value(df_ma.loc[date_1d:date_1d, 'price'])
        price_2d = clean_df_value(df_ma.loc[date_2d:date_2d, 'price'])
        price_3d = clean_df_value(df_ma.loc[date_3d:date_3d, 'price'])
        price_1w = clean_df_value(df_ma.loc[date_1w:date_1w, 'price'])
        price_2w = clean_df_value(df_ma.loc[date_2w:date_2w, 'price'])
        price_4w = clean_df_value(df_ma.loc[date_4w:date_4w, 'price'])
        price_8w = clean_df_value(df_ma.loc[date_8w:date_8w, 'price'])
        ...
        # beta
        my_sql = f"""SELECT entry_date,beta,alpha FROM product_beta WHERE product_id ={drop.product.id} 
    ...
    df.to_csv("H:\Python Output\drop_price.csv", index=False)
